...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        $data['table'] = App_table::find('clients');
         ...
         // 🔽 Load Affiliate Model and get data
    $this->load->model('affiliate/affiliate_model'); // Adjust the path if it's inside a module
    $data['affiliate_members'] = $this->affiliate_model->get_member(); // Get all affiliate users
    ...
    public function client($id = '')
{
    ...
    if ($this->input->post() && !$this->input->is_ajax_request()) {
        if ($id == '') {
            ...
            // 🔽 Added affiliate logic with logs
            if (isset($data['affiliate_user_id']) && !empty($data['affiliate_user_id'])) {
                log_message('debug', 'Affiliate user ID received: ' . $data['affiliate_user_id']);

                $this->db->where('id', $data['affiliate_user_id']);
                $affiliate = $this->db->get(db_prefix() . 'affiliate_users')->row();

                if ($affiliate) {
                    log_message('debug', 'Affiliate found: ' . print_r($affiliate, true));
                    $data['affiliate_code'] = $affiliate->affiliate_code;
                    log_message('debug', 'Affiliate code set in client data: ' . $affiliate->affiliate_code);
                } else {
                    log_message('debug', 'Affiliate not found for ID: ' . $data['affiliate_user_id']);
                }
            } else {
                log_message('debug', 'No affiliate_user_id provided in form data.');
            }
            ...
            log_message('debug', 'Client added with ID: ' . $id);

            if (staff_cant('view', 'customers')) {
                $assign['customer_admins']   = [];
                $assign['customer_admins'][] = get_staff_user_id();
                $this->clients_model->assign_admins($assign, $id);
                log_message('debug', 'Assigned current staff user as customer admin.');
            }
            ...
            $data = $this->input->post();
            unset($data['isedit']); // Add this line

            $success = $this->clients_model->update($data, $id);
            log_message('debug', 'Client update status for ID ' . $id . ': ' . ($success ? 'Success' : 'Failed'));
    ...
    // 🔽 Add affiliate user data for dropdown in view
    $data['affiliate_members'] = $this->db->where('status', 1)->get('tblaffiliate_users')->result_array();
