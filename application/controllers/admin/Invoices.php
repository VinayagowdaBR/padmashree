...
public function validate_invoice_number()
{
    ...
    // When editing, allow if invoice number didn't change
    if ($isedit == 'true') {
    ...
    // Generate formatted number with date + padded number (e.g. 250604001)
    $date_part = date('ymd', strtotime($date));  // Format date as yymmdd
    $padded_number = str_pad(ltrim($number, '0'), 3, '0', STR_PAD_LEFT);
    $formatted_number = $date_part . $padded_number;

    // Check if an invoice with this formatted number exists (and is not draft)
    $exists = total_rows('invoices', [
        'formatted_number' => $formatted_number,
        'status !=' => Invoices_model::STATUS_DRAFT,
    ]) > 0;

    if ($exists) {
        echo 'false';  // Invoice number already exists
    } else {
        echo 'true';   // Unique invoice number, allowed
    }
}
