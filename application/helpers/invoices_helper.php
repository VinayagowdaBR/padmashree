...
function format_invoice_number($id)
{
    ...
    // Fetch the invoice if $id is not already an object
    if (!is_object($id)) {
        $CI->db->select('id, date, number, prefix, number_format, status');
    ...
    // If no invoice is found, return an empty string
    if (!$invoice) {
        log_message('error', 'format_invoice_number: Invoice not found for ID ' . $id);
        return '';
    }
    ...
    // If invoice is in draft status, return 'DRAFT'
    if ($invoice->status == Invoices_model::STATUS_DRAFT) {
        $number = $invoice->prefix . 'DRAFT';
        log_message('error', 'format_invoice_number: Invoice is in DRAFT status');
    } else {
        ...
        if ((int) $invoice->number_format === 5) {
            ...
            log_message('error', 'format_invoice_number: Using default format - Result: ' . $number);
        }
    }

    // Final output
    log_message('error', 'format_invoice_number: Final formatted number - ' . $number);

    // Apply filters and return formatted invoice number
    return hooks()->apply_filters('format_invoice_number', $number, [
        'id'      => $id,
        'invoice' => $invoice,
    ]);
