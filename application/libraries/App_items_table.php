  ...
  public function items()
{
    ...
    $numberWidth         = 5;
    $descriptionWidth    = 30;
    $longDescriptionWidth =45;
    $rateWidth           = 20;

    if ($this->for == 'html') {
        $descriptionWidth    -= 2;
        $longDescriptionWidth -= 2;
        $rateWidth           -= 2;
    }
    ...
    foreach ($this->items as $item) {
        ...
        // Number column
        $itemHTML .= '<td' . $this->td_attributes() . ' align="center" width="' . $numberWidth . '%">' . $i . '</td>';
  
        // Description column
            $itemHTML .= '<td style="" align="left" width="' . $descriptionWidth . '%">';
            if (!empty($item['description'])) {
                $itemHTML .= '<span style="font-size:' . $this->get_pdf_font_size() . 'px;"><strong>'
                    . e($this->period_merge_field($item['description'])) . '</strong></span>';
            }
            $itemHTML .= '</td>';

            // Long description column
            $itemHTML .= '<td style=" text-align: center;" width="' . $longDescriptionWidth . '%">';
            if (!empty($item['long_description'])) {
                $itemHTML .= '<span style="color:#424242; text-align: left;margin-left:20px; display: block;">'
                    . e($this->period_merge_field($item['long_description'])) . '</span>';
            }
            $itemHTML .= '</td>';
        ...
        $itemHTML .= '<td align="right" width="' . $rateWidth . '%">' . e($rate) . '</td>';
...
{
    $html = '<tr>';
    $html .= '<th align="center" width="5%">'. $this->number_heading() .'</th>';
    $html .= '<th class="description" width="30%" align="left">' . $this->item_heading() . '</th>';
    $html .= '<th class="description" width="40%" align="left">Investigation</th>';
    $html .= '<th align="right" width="25%">' . $this->rate_heading() . '</th>';
    $html .= '</tr>';
 ...
 public function pdf_headings()
{
    ...
    $tblhtml .= '<th width="30%" align="left">' . $this->item_heading() . '</th>';
    $tblhtml .= '<th width="55%" align="left">Investigation</th>';
