...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            'company',
            'CONCAT(' . db_prefix() . 'contacts.firstname, " ", ' . db_prefix() . 'contacts.lastname) as fullname', // Fixed CONCAT
            db_prefix() . 'contacts.email as email', // Make sure this is included correctly
            ...
            'CONCAT(' . db_prefix() . 'affiliate_users.firstname, " ", ' . db_prefix() . 'affiliate_users.lastname) as affiliate_user_name', // Fixed CONCAT
        ];
        ...
        $join = [
            ...
            'LEFT JOIN ' . db_prefix() . 'affiliate_users ON ' . db_prefix() . 'affiliate_users.affiliate_code = ' . db_prefix() . 'clients.affiliate_code COLLATE utf8mb4_unicode_ci', // Ensure correct collation
        ];
        ...
        // log_message('debug', 'SQL Query: ' . $this->ci->db->last_query());
        ...
        // log_message('debug', 'Executed SQL Query: ' . $this->ci->db->last_query());

        // Log the result for debugging
        // log_message('debug', 'Query Result: ' . print_r($result['rResult'], true));
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
            ...
            $row[] = !empty($aRow['affiliate_user_name']) ? e($aRow['affiliate_user_name']) : 'N/A';  // Fallback to 'N/A' if empty
