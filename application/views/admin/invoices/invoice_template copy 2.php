...
if ($format == 1) {
    ...
    // Format 5: YYMMDD + 3-digit invoice number
    $yymmdd = date('ymd');
    if (isset($invoice)) {
...
?>
                ...
                <?php } ?>
                <?php
$allowed_fields = ['Age', 'Sex', 'Mobile.no', 'AgeOption'];
...
foreach ($xpath->query('//label') as $label) {
    ...
    $clean_text = ltrim($label_text, "* "); // remove * or extra spaces
    $all_labels[] = $label_text; 

    if (in_array($clean_text, $allowed_fields)) {
        $field_html = $dom->saveHTML($label->parentNode);

        if ($clean_text === 'Age') {
            $age_html = $field_html;
        } elseif ($clean_text === 'AgeOption') {
            $age_option_html = $field_html;
        } else {
            $filtered_html .= $field_html; // add normally
        }
    }
}

if ($age_html || $age_option_html) {
    $filtered_html .= "<div style='display:flex; gap:15px; align-items:center;'>"
        . $age_html 
        . $age_option_html 
        . "</div>";
}
...
// Debug log
log_message('debug', 'Custom fields found: ' . implode(', ', $all_labels));
            ...
            <div class="col-md-6">
                <div class="tw-ml-3">
                    ...
                    <div
                        ...
                        <br />
                        <?php
                        log_message('debug', 'Payment Modes IN INVOICE TEMPETE: ' . print_r($payment_modes, true));
...
foreach ($custom_fields as $cf) {
    if (strtolower($cf['name']) === 'commission') {
        continue; // Skip commission header entirely
    }
...
<!-- <th width="15%" align="right">
                ...
                <tbody>
                    <tr class="main">
                        ...
                        <?php
$customFieldsOutput = render_custom_fields_items_table_add_edit_preview();

// Match all <td>...</td> blocks
preg_match_all('/<td.*?<\/td>/si', $customFieldsOutput, $matches);

// Take only the first one
$firstTd = $matches[0][0] ?? '';

// Log it
log_message('debug', 'First TD Only: ' . $firstTd);

// Show only the first <td>
echo $firstTd;
?>
...
$select = '<select class="selectpicker display-block tax main-tax" data-width="100%" name="taxname" multiple data-none-selected-text="' . _l('no_tax') . '" style="display: none !important;">';
...
?>
        ...
        onclick="
            ...
            add_item_to_table('undefined','undefined',<?= e($new_item); ?>);
            return false;"
