...
private function get_daily_incremental_invoice_number()
{
    ...
    // Return yymmdd + 3-digit serial number (e.g., 250603001)
    return   str_pad($count_today + 1, 3, '0', STR_PAD_LEFT);
}
...
//     // Check if an invoice already exists for this customer
//     $this->db->where('clientid', $customer_id);
//     $this->db->limit(1);  // Limit to 1 invoice
//     $existing_invoice = $this->db->get(db_prefix() . 'invoices')->row();
...
//     // ✅ Set invoice number based on date if using custom format 5
//     if (isset($data['status']) && $data['status'] == self::STATUS_DRAFT) {
//         $data['number'] = self::STATUS_DRAFT_NUMBER;
//     } else {
...
//             $this->db->where('number_format', 5);
//             $max_result = $this->db->get()->row();

//             $next_number = isset($max_result->max_number) && $max_result->max_number ? $max_result->max_number + 1 : 1;

//             $data['number'] = $next_number;
//             $data['date'] = $today;
//         } else {
//             $data['number'] = $this->get_next_invoice_number();
//         }
//     }
...
//         foreach ($items as $key => $item) {
//             if ($itemid = add_new_sales_item_post($item, $insert_id, 'invoice')) {
//                 // Related tasks/expenses can be handled here
//             }
//         }
