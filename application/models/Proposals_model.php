...
class Proposals_model extends App_Model
{
    ...
    {
        log_message('debug', 'Starting add proposal process');
    
        $data['allow_comments'] = isset($data['allow_comments']) ? 1 : 0;
    
        log_message('debug', 'Allow comments set: ' . $data['allow_comments']);
        
        $save_and_send = isset($data['save_and_send']);
        $tags = isset($data['tags']) ? $data['tags'] : '';
    
        log_message('debug', 'Save and send flag: ' . ($save_and_send ? 'true' : 'false'));
        log_message('debug', 'Tags: ' . $tags);
    
        if (isset($data['custom_fields'])) {
            $custom_fields = $data['custom_fields'];
            unset($data['custom_fields']);
            log_message('debug', 'Custom fields set');
        }
        ...
        if (isset($data['estimate_request_id'])) {
            ...
            log_message('debug', 'Estimate request ID: ' . $estimateRequestID);
        }
    
        $data['address'] = trim($data['address']);
        $data['address'] = nl2br($data['address']);
    
        log_message('debug', 'Address formatted: ' . $data['address']);
    
        $data['datecreated'] = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['hash']        = app_generate_hash();
    
        log_message('debug', 'Date created: ' . $data['datecreated']);
        log_message('debug', 'Added from staff ID: ' . $data['addedfrom']);
        ...
        if (isset($data['newitems'])) {
            $items = $data['newitems'];
            unset($data['newitems']);
            log_message('debug', 'Items set: ' . count($items));
        }
    
        if ($this->copy == false) {
            $data['content'] = '{proposal_items}';
            log_message('debug', 'Content set to {proposal_items}');
        }
    
        if (isset($data['rel_id'], $data['rel_type']) && $data['rel_type'] !== 'customer') {
            $data['project_id'] = null;
            log_message('debug', 'Project ID set to null for non-customer relation type');
        }
    
        // ✅ Generate MRD and add to data
        $data['mrd_no'] = $this->generate_mrd_number();
        log_message('debug', 'Generated MRD: ' . $data['mrd']);
        ...
        if ($insert_id) {
            log_message('debug', 'Proposal inserted with ID: ' . $insert_id);
            ...
            if ($estimateRequestID !== false && $estimateRequestID != '') {
                ...
                log_message('debug', 'Estimate request updated with status ID: ' . $completedStatus->id);
            }
    
            if (isset($custom_fields)) {
                handle_custom_fields_post($insert_id, $custom_fields);
                log_message('debug', 'Custom fields saved');
            }
    
            handle_tags_save($tags, $insert_id, 'proposal');
            log_message('debug', 'Tags saved for proposal ID: ' . $insert_id);
    
            foreach ($items as $key => $item) {
                if ($itemid = add_new_sales_item_post($item, $insert_id, 'proposal')) {
                    _maybe_insert_post_item_tax($itemid, $item, $insert_id, 'proposal');
                    log_message('debug', 'Sales item added for proposal ID: ' . $insert_id);
                }
            }
            ...
            if ($proposal->assigned != 0) {
                if ($proposal->assigned != get_staff_user_id()) {
                    ...
                    if ($notified) {
                        ...
                        log_message('debug', 'Notification sent to user ID: ' . $proposal->assigned);
                    }
                }
            }
            ...
            if ($data['rel_type'] == 'lead') {
                ...
                log_message('debug', 'Lead activity logged for proposal ID: ' . $insert_id);
            }
    
            update_sales_total_tax_column($insert_id, 'proposal', db_prefix() . 'proposals');
            log_message('debug', 'Sales total tax updated for proposal ID: ' . $insert_id);
    
            log_activity('New Proposal Created [ID: ' . $insert_id . ']');
            log_message('debug', 'Activity log recorded for proposal creation');
    
            if ($save_and_send === true) {
                $this->send_proposal_to_email($insert_id);
                log_message('debug', 'Proposal sent to email');
            }
    
            hooks()->do_action('proposal_created', $insert_id);
            log_message('debug', 'Proposal created hook triggered');
    
            return $insert_id;
        }
    
        log_message('debug', 'Proposal creation failed');
        return false;
    }

    private function generate_mrd_number()
{
    $today = date('Ymd'); // e.g., 20250412
    $this->db->like('mrd_no', 'MRD-' . $today, 'after');
    $this->db->order_by('id', 'DESC');
    $this->db->limit(1);
    $last = $this->db->get(db_prefix() . 'proposals')->row();

    $next_number = 1;
    if ($last && isset($last->mrd_no)) {
        preg_match('/(\d+)$/', $last->mrd_no, $matches);
        if (!empty($matches[1])) {
            $next_number = (int)$matches[1] + 1;
        }
    }

    return 'MRD-' . $today . str_pad($next_number, 4, '0', STR_PAD_LEFT);
}
