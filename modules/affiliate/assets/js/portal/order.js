...
(function ($) {
  ...
  $("body").on("change", 'select[name="customer"]', function () {
    ...
    requestGetJSON("affiliate/usercontrol/client_change_data/" + val).done(
      function (response) {
        ...
        for (var fsd in billingAndShippingFields) {
          if (billingAndShippingFields[fsd].indexOf("shipping") > -1) {
            if (billingAndShippingFields[fsd].indexOf("country") > -1) {
              ...
              if (
                billingAndShippingFields[fsd].indexOf("shipping_street") > -1
              ) {
      ...
      if (response.success == true) {
        ...
        if ($("body").find(".accounting-template").length > 0) {
          if (!item_select.hasClass("ajax-search")) {
            var group = item_select.find(
              '[data-group-id="' + response.item.group_id + '"]'
            );
            if (group.length == 0) {
              var _option =
                '<optgroup label="' +
                (response.item.group_name == null
                  ? ""
                  : response.item.group_name) +
                '" data-group-id="' +
              ...
              group.prepend(
                '<option data-subtext="' +
                  response.item.long_description +
                  '" value="' +
                  response.item.itemid +
                  '">(' +
                  accounting.formatNumber(response.item.rate) +
                  ") " +
          ...
          if (!item_select.hasClass("ajax-search")) {
            ...
            item_select
              ...
              .filter(function () {
                return (
                  !$(this).is(".newitem") && !$(this).is(".newitem-divider")
                );
              })
              .remove();
            ...
            $("body")
              .find(".items-select-wrapper")
              .append(clonedItemsAjaxSearchSelect);
            init_ajax_search(
              "items",
              "#item_select.ajax-search",
              undefined,
              site_url + "affiliate/usercontrol/search_item"
            );
          }
...
function init_item_js() {
  ...
  $("body").on("show.bs.modal", "#sales_item_modal", function (event) {
    ...
    if (typeof id !== "undefined") {
      ...
      requestGetJSON("affiliate/usercontrol/get_item_by_id/" + id).done(
        function (response) {
          $itemModal
          ...
          $itemModal
            ...
            .val(response.long_description.replace(/(<|<)br\s*\/*(>|>)/g, " "));
          ...
          $('select[name="tax2"]')
            .selectpicker("val", response.taxid_2)
            .change();
...
function add_item_to_preview(id) {
  "use strict";
  requestGetJSON("affiliate/usercontrol/get_item_by_id/" + id).done(function (
    response
  ) {
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  console.trace("add_item_to_table invoked");
  ("use strict");
  // If not custom data passed get from the preview
  console.log(
    "add_item_to_table invoked with:",
    data,
    itemid,
    merge_invoice,
    bill_expense
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row += '<td class="taxrate order">' + tax_dropdown + "</td>";
    table_row +=
...
function get_taxes_dropdown_template(name, taxname) {
  ...
  var d = $.post(
    site_url + "affiliate/usercontrol/get_taxes_dropdown_template/",
    {
      name: name,
      taxname: taxname,
