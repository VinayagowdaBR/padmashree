...
class Affiliate_model extends App_Model
{
    ...
    {
        log_message('info', 'add_member called with data: ' . print_r($data, true));
        ...
        log_message('info', 'Insert result ID: ' . $insert_id);
    ...
    public function update_member($data, $id)
{
    ...
    // Only hash password if it's actually set
    ...
    public function get_product_group_select()
    {
        ...
        $items_groups     = $this->db->get(db_prefix() . 'items')->result_array();
        $list_item_groups = [];
        foreach ($items_groups as $key => $group) {
            $note               = [];
            $note['id']         = $group['id'];
            $note['label']      = $group['description'];
    ...
    public function get_product_select($where = [])
    {
        ...
        $this->db->where(key: 'can_be_sold = "1"');
        $items     = $this->db->get(db_prefix() . 'items')->result_array();
        $list_item = [];
        foreach ($items as $key => $item) {
            $note          = [];
            $note['id']    = $item['id'];
            $note['label'] = $item['long_description'];
            $list_item[]   = $note;
        }
        return $list_item;
    }
    ...
    {
        $this->db->insert('tblaffiliate_program_categorys', $data);
