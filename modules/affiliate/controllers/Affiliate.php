...
public function member($id = '')
{
    ...
    public function member_group()
    {
        ...
        if ($data) {
            if ($data['id'] == '') {
                ...
                set_alert($id ? 'success' : 'warning', $message);
            } else {
                if (!affiliate_has_permission('settings', '', 'edit')) {
                    access_denied('settings');
                }
                $success = $this->affiliate_model->edit_member_group($data);
                $message = $success ? _l('updated_successfully', _l('member_group')) : _l('not_updated');
                set_alert($success ? 'success' : 'warning', $message);
            }
    
            redirect(admin_url('affiliate/settings?group=member_group')); // or wherever you list groups
        }
    }
    ...
    public function affiliate_member_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $select = [
                'affiliate.id as id',
                'affiliate.firstname as affiliate_firstname',
                'affiliate.lastname as affiliate_lastname',
                // 'affiliate.country as affiliate_country',
                // 'affiliate.email as affiliate_email',
                // 'affiliate.username as affiliate_username',
                'affiliate.phone as affiliate_phone',
                // 'affiliate.vendor_status as affiliate_vendor_status',
                // 'CONCAT(sponsor.firstname, " ", sponsor.lastname) AS sponsor'
            ];
    
            $where = ['AND affiliate.approval = 1'];
    
            $aColumns     = $select;
            $sIndexColumn = 'id';
            $sTable       = db_prefix() . 'affiliate_users as affiliate';
    
            $join         = [
                'LEFT JOIN ' . db_prefix() . 'affiliate_users as sponsor ON sponsor.id = affiliate.under_affiliate'
            ];
            ...
            foreach ($custom_fields as $key => $field) {
                ...
                array_push($join, 'LEFT JOIN ' . db_prefix() . 'customfieldsvalues as ctable_' . $key . ' ON affiliate.id = ctable_' . $key . '.relid AND ctable_' . $key . '.fieldto="' . $field['fieldto'] . '" AND ctable_' . $key . '.fieldid=' . $field['id']);
            }
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = '<div class="checkbox"><input type="checkbox" value="' . $aRow['affiliate_email'] . '"><label></label></div>';
    
                $row[] = $aRow['affiliate_firstname'] . ' ' . $aRow['affiliate_lastname']; // Full Name
    
                // $country_name = '';
                // if ($aRow['affiliate_country'] != '') {
                //     $country = get_country($aRow['affiliate_country']);
                ...
                // $row[] = $aRow['affiliate_email']; // Email
                // $row[] = $aRow['affiliate_username']; // Username
                // $row[] = $aRow['sponsor'] ?: 'No Sponsor'; // Sponsor
                $row[] = $aRow['affiliate_phone']; // Phone
                // $row[] = $aRow['affiliate_vendor_status']; // Vendor
    ...
    public function registration_list_table()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->db->from($sTable);
            $this->db->where('(approval != 1 or approval is null)');
            $query = $this->db->get();
    
            log_message('debug', 'SQL Query: ' . $this->db->last_query());
            log_message('debug', 'Query Result: ' . print_r($query->result(), true));
    
            // Check if any data exists
            if ($query->num_rows() == 0) {
                log_message('debug', 'No data found for affiliate_users table.');
            }
    
            // Assuming the data is fetched correctly now, you can process it as usual
    ...
    public function program_category()
    {
        if ($this->input->method(true) === 'POST') {
            ...
            if (empty($data['name'])) {
                set_alert('warning', _l('name_required', _l('program_category')));
            ...
            if (empty($data['id'])) {
                ...
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('program_category')));
                } else {
                    set_alert('danger', _l('failed_to_add', _l('program_category')));
                }
            } else {
                $success = $this->affiliate_model->edit_program_category($data);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('program_category')));
                } else {
                    set_alert('danger', _l('failed_to_update', _l('program_category')));
                }
            }
    ...
    public function update_setting()
    {
        ...
        $success = $this->affiliate_model->update_setting($data);
    
        if ($success) {
            set_alert('success', _l('updated_successfully', _l('general_settings')));
        } else {
