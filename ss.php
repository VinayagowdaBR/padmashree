...
// Show first field normally
if (!empty($matches[0][0])) {
    $output .= $matches[0][0];
}

// Hide second field (keep in DOM but not visible)
if (!empty($matches[0][1])) {
    // safer regex: non-greedy match
    $secondTd = preg_replace('/<td(.*?)>/i', '<td$1 style="display:none;">', $matches[0][1]);
    $output .= $secondTd;
}

// Log
log_message('debug', 'Modified TDs Output: ' . $output);
